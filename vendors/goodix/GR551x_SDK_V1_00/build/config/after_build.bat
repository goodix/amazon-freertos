...
@if "%2"=="" (
echo please pass the keil install path !!!!!!
@set BIN_PATH=C:\Keil_v5\ARM\ARMCC\Bin
@set BUILD_PATH=C:\Keil_v5\ARM
) else (
@set BIN_PATH=%2\ARM\ARMCC\Bin
@set BUILD_PATH=%2\ARM
)
...
@set TOOL_PATH=%BUILD_PATH%\PACK\Goodix\GR551x\1.0.0\build\binaries
...
@set FW_ENCRYPT_SIGN_TOOL_PATH=%BUILD_PATH%\PACK\Goodix\GR551x\1.0.0\build\binaries\encrypt_tools
...
%TOOL_PATH%\ble_tools.exe  --cfg=..\Src\config\custom_config.h --mode=gen --bin=%OBJ_DIR_PATH%\%TARGET%.bin --outdir=%OUTPUT_DIR_PATH% --app_name=%TARGET_APP%  --dap
...
%TOOL_PATH%\ble_tools.exe --cfg=%BUILD_PATH%\PACK\Goodix\GR551x\1.0.0\build\scripts\simulation_unenc.txt --mode=merge_bin --input_path=%OUTPUT_DIR_PATH% --bin=%OUTPUT_DIR_PATH%\%TARGET_APP%_sim.bin  --out_bin_size=0x80000 >nul
