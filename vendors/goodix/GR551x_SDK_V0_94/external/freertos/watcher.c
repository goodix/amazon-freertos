...
static dual_tim_handle_t DUALTIM0Handle;
...
{
	return ll_dual_tim_get_counter(DUALTIM0Handle.p_instance);
}

void watcher_timer_init()
{
	DUALTIM0Handle.p_instance = DUAL_TIM0;
    DUALTIM0Handle.init.prescaler   = DUAL_TIM_PRESCALER_DIV16;
    DUALTIM0Handle.init.counter_mode = DUAL_TIM_COUNTERMODE_LOOP;
    DUALTIM0Handle.init.auto_reload  =  (unsigned int)-1;
    hal_dual_tim_init(&DUALTIM0Handle);
    hal_dual_tim_start_it(&DUALTIM0Handle);
}
